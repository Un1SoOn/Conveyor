...
/** Логика расчета предложения при отсутствии страховки и наличии клиентства в банке*/
@Service
@RequiredArgsConstructor
public class AlgorithmNotInsureNotSalaryImpl implements Algorithm {
    /** Маппер*/
    ...
    private static final double ADDITIONAL_PERCENT_TO_RATE = 15;
    ...
    public LoanOfferDTO calcOffer(LoanApplicationRequestDTO applicationRequestDTO) {
        ...
        /** Ежемесячный платеж*/
        BigDecimal monthlyPayment = BigDecimal.valueOf((applicationRequestDTO.getAmount() + applicationRequestDTO.getAmount()
                * (rate / 100 + ADDITIONAL_PERCENT_TO_RATE / 100)) / applicationRequestDTO.getTerm()).setScale(1, RoundingMode.HALF_UP);
        ...
        /** Не является зарплатными клиентом банка*/
